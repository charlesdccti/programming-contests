...
int main()
{
    ...
    {
        if(m == 12) {
            if(d == 24) { cout << "E vespera de natal!\n"; }
            if(d == 25) { cout << "E natal!\n";}
            if(d > 25) { cout << "Ja passou!\n";}
            if(d < 24) { cout << "Faltam " << 25-d << " dias para o natal!\n";}
        } else {
            int x = d;
            int y = 0;
            while(m != 12) {
                while(x != months[m-1]){ y++; x++; }
                m++;
                x = 0;
            }
            cout << "Faltam " << 25+y << " dias para o natal!\n";
        }
    }
